...
function Monitor(){
  ...
  this.objectsLeft = Object.keys(this.hosts).length +
    Object.keys(this.sites).length +
    Object.keys(this.dns).length

  this.emit("debug", "Monitor have %s objects to check", this.objectsLeft);
...
Monitor.prototype.done = function (label) {
  this.objectsLeft--;
  this.emit("objectDone", label);
...
M.on("objectDone", function (label) {
  console.log("[done] Object %s done: %s left", label, M.objectsLeft);
});

M.on("end", function () {
  console.log("[end] Everything OK");
});
